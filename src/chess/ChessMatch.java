...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		ChessPiece movedPiece = (ChessPiece)board.piece(target); //A peça que se moveu foi a peça que estava no destino
		...
		check = (testCheck(opponent(currentPlayer))) ? true : false; //Se testecheck do oponent do jogador atual for verdade, a partida está em xeque, se não, a partida não está em xeque
	...
	private Piece makeMove(Position source, Position target) {
		ChessPiece p = (ChessPiece)board.removePiece(source); //Retirou a peça que estava na posição de origem
		p.increaseMoveCount();
	...
	//Operação para desfazer o movimento
	...
	//Operação para verificar se o Rei está em xeque
	...
	private boolean testCheckMate(Color color) {
		...
		List<Piece> list = piecesOnTheBoard.stream().filter(x -> ((ChessPiece)x).getColor() == color).collect(Collectors.toList()); //Todas as peças da cor
		for (Piece p : list) {
			boolean[][] mat = p.possibleMoves();
